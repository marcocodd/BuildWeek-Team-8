window.addEventListener("load", () => {
  ...
  const correctPercNumberTarget =
    correctAnswers === 0
      ? 0
      : Math.floor((correctAnswers / total_questions) * 100);
  ...
  details.forEach((detail, index) => {
    ...
    let headerIcon =
      detail.given_answer === detail.correct_answer ? iconList[3] : iconList[2];
    detailTitle.innerHTML = `(${index + 1}) <i class="${headerIcon}"></i> ${
      detail.question
    }`;
